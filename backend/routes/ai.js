 ...
 */
router.post("/admin-assistant", authenticateToken, requireRole("DEPARTMENT_ADMIN", "SUPER_ADMIN", "OFFICER"), (req, res) => {
  try {
    ...
    const categoryBreakdown = db.prepare("SELECT category, COUNT(*) as count FROM grievances GROUP BY category ORDER BY count DESC").all();
    const departmentBreakdown = db.prepare("SELECT department, COUNT(*) as count FROM grievances GROUP BY department ORDER BY count DESC").all();
