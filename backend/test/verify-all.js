...
async function runFullVerificationSuite() {
  ...
  const citizen = db.prepare("SELECT * FROM users WHERE role = 'CITIZEN' LIMIT 1").get();
  const officer = db.prepare("SELECT * FROM users WHERE role = 'OFFICER' LIMIT 1").get();
  ...
  try {
    ...
    db.prepare("UPDATE grievances SET sla_deadline = ? WHERE id = ?").run(pastSLA, grvId);

    const count = processSLAEscalations();
    assert(count > 0, "Must detect past SLA breach");

    const updated = db.prepare("SELECT * FROM grievances WHERE id = ?").get(grvId);
  ...
  try {
    ...
    const reopened = db.prepare("SELECT * FROM grievances WHERE id = ?").get(grvId);
  ...
  try {
    const grv = db.prepare("SELECT id FROM grievances LIMIT 1").get();
